...
curl --silent --write-out "Het pTTPSTATUS:%{http_code}" -X PUT $CONNECTOR_ENDPOINT -H 'Content-Type: application/json' -H 'Accept: application/json' --data '@/'"$CONNECTOR_CONFIG_DIR"'/'"$CONNECTOR_NAME"'.json'